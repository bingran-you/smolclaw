"""CLI: smolclaw-gcal serve|seed|reset|run-task."""
...
@click.group()
@click.option("--db", default="claw_gcal.db", help="Path to SQLite database file")
@click.pass_context
def cli(ctx, db):
...
@click.option("--port", default=8002, type=int, help="Bind port")
...
def serve(ctx, host, port, no_mcp):
    ...
    from claw_gcal.server import run_server
...
def reset(ctx):
    ...
    from claw_gcal.state.snapshots import restore_snapshot
    ...
    if success:
        ...
        click.echo("Error: No initial snapshot found. Run `smolclaw-gcal seed` first.", err=True)
        sys.exit(1)
...
def run_task(ctx, task_name, scenario):
    ...
    click.echo(f"  smolclaw-gcal eval-task {task_name}")
...
def eval_task(ctx, task_name):
    ...
    from claw_gcal.state.snapshots import get_diff, get_state_dump
    from claw_gcal.tasks import get_task
